...
//卡表对象，继承于Generation对象
class CardGeneration: public Generation {
  friend class VMStructs;
 protected:
  // This is shared with other generations. 与其他Generation实例共享的卡表实现实例
  GenRemSet* _rs;
  // This is local to this generation. 当前Generation独享的BlockOffsetArray实现
  BlockOffsetSharedArray* _bts;

  // current shrinking effect: this damps shrinking when the heap gets empty. 老年代内存缩容的百分比
  size_t _shrink_factor;

  size_t _min_heap_delta_bytes;   // Minimum amount to expand.  老年代内存扩展或者缩容时的最低内存值
